...
There is one way to lower the number without lowering the tangle, and it has to be said here rather than found later. The ordered-operand rule charges per operand when a run holds a call. Move that run into a named helper and the caller now holds one call, which costs one, while the operands it was charged for live in a method the caller's number never sees. The reader still traces the same logic; the caller's figure falls. The scorer cannot catch this, because it measures one method at a time from its own syntax tree, and a scorer that followed calls would be a different piece of software with its own wrong answers. So the blind spot stays in the scorer and the check lives in the loop: UntangleIt reports what the pieces come to together as well as what the worst piece comes to, and a split that only spread the work shows in the total. The measured case is this toolkit's own complexity library at 1.0.9, where `exclusiveKey` in python.ts fell from 31 to 4 while the eight methods it became came to 24. Most of that drop was nesting genuinely removed. Not all of it was.
