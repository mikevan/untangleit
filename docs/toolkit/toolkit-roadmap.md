...
| 1.0 $${\color{green}\text{(Done)}}$$ | The JavaScript frameworks that allow testing | React, Vue, and Svelte under Jest or Vitest, with `.vue` and `.svelte` parsed on their real lines; Angular through `ng test` with Vitest or Karma; Mocha and Playwright component tests through Witness, the toolkit's own instrumentation | The largest user base the toolkit has, about 4.8M monthly contributors across TypeScript and JavaScript. Opened by the v1.0.0 tag and closed at 1.0.8, eight patch builds: detection, single-file components, two Angular drivers, and Witness (DeepTest's docs/witness.md), which owns the loader, the instrumenter, and the page for the runners no coverage package could serve. |
