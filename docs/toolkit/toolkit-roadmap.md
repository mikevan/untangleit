...
Two consequences. The top three languages are the two the toolkit already handles, and TypeScript plus JavaScript is about 4.8 million monthly contributors, the largest base the toolkit has by a wide margin; so the JavaScript framework runner work, which serves that base and lands on plugins that already exist, comes first, as patches, before any new language. And C++ is not in the top five, so the 1.3 slot is decided between C++ and Go on the numbers at the time, not promised now.
...
| 1.0.x | (not a language) the JavaScript frameworks that allow testing | React, Vue, Svelte under Jest or Vitest (mostly working today); Angular under Karma or Jest; Mocha; Playwright component tests | The largest user base the toolkit has, about 4.8M monthly contributors across TypeScript and JavaScript, and the work is detection plus runner adapters inside the existing plugin. Patches, starting now, before Java begins. |
...
The JavaScript frameworks that allow testing (React, Vue, Angular, Svelte, and the runners under them: Mocha, Karma, Playwright component tests) are not a language and do not take a minor number. Most of them work today because the code is TypeScript or JavaScript and the tests run under Jest or Vitest; what is missing is detection and a few more runner adapters, and Angular's Karma path has no Istanbul hook yet. That is runner work inside the existing plugin, and it ships as 1.0.x patches starting now, because that is where the users are (see "Who the users are"). It also proves the runner layer generalises before Java and C# need it.
...
- The major number moves when the inter-tool protocol changes incompatibly (toolkit-api.md, section 7) or when a verb is redefined. There is no plan for that.
- Tags mark these known-good points only: `v1.0.0` on every repo on 2026-09-12, then `v1.1.0` when Java lands, and so on. Routine work is never tagged.
