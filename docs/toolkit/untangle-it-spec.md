...
The tool carried a different name through 0.1.3. The Marketplace upload was refused: "Similar extension display name: refactorix already exists." A dormant Visual Studio extension by mynkow (C# helpers, last active around 2017) also shares that first name on the Marketplace website. The tool had never been published, so the change cost nothing: UntangleIt, `prs.untangleit`, commands `untangleit.*`, records `.untangleit/`, settings `untangleit.*`, folder `C:\workspace\UntangleIt`, repo `mikevan/untangleit`. "Untangle" was the tool's verb in the architecture document from draft 1, so the name says what the tool does in Jeff's words.

## Decision recorded 2026-09-06

Refactoring is its own tool. DeepTest's `fixFunction` refactor branch and the refactor mode of `buildFunctionBrief` (src/report/brief.ts and src/decisions/commands.ts in DeepTest) are UntangleIt's seed. They are to be pulled out into this tool and fixed here, not patched in DeepTest. DeepTest keeps the question ("visitStatement() has 48 ways through it") and the door ("Break it into smaller pieces"); behind the door will be `untangleit.method`.

## The first field result, and what it teaches

Run against DeepTest's own `visitStatement()` (48 ways through, limit 10), the seed brief produced a refactor that reduced the count by 10 instead of down to 10. The brief said "at most 10 ways through" in plain words and the assistant still did the wrong arithmetic. Lesson for the design: a brief is not a loop. UntangleIt must measure after every transform and go again until every piece is within the limit, and it must report the numbers per piece; it cannot trust the assistant to have understood the target, however plainly it was stated. This is the strongest argument for the mechanical engine measuring, and for the loop below being enforced by the tool rather than described to the assistant.

## The verb

UntangleIt untangles one method at a time. Its whole job: take a method whose complexity is above the limit, and leave behind a set of methods that do exactly the same thing, each within the limit. It does not test (DeepTest does), it does not remember (KeepSafe does), and it does not decide (the human does).
...
3. Confirm. One modal sentence: what is about to happen, who does the work, and that UntangleIt measures afterwards and does not accept the result for anyone.
...
5. Verify. Run the project's own test suite through the project's own runner, exactly as DeepTest does. Existing tests must pass unedited. Then measure every piece. If any piece is still over the limit, go back to step 4 for that piece, up to a configurable number of rounds; then report. Report per piece: name, ways through, within the limit or not. Report the total as well: what the pieces come to together in tangle, against what the method and any piece the untangling disturbed were carrying before. Every piece can fit under the limit while the total holds or rises, which means the tangle was spread rather than removed, and the card says that in those words. The scorer cannot catch it on its own, because it measures one method at a time and a boolean run moved into a helper costs the caller one call instead of its operands; the total is where it shows. UntangleIt still does not decide: it says both numbers and the person keeps or restores.
6. Decide. Keep it, or restore the checkpoint. UntangleIt never restores on its own.
...
- `untangleit.method` (interactive) with `{ path, startLine }`: run the loop on one method. This is what DeepTest's "Break it into smaller pieces" calls when UntangleIt is installed.
- `untangleit.worst` (interactive): the loop on the workspace's most tangled method.
- `untangleit.api.measure` (silent) with `{ path }`: methods in the file with their ways through.
- `untangleit.api.plan` (silent) with `{ path, startLine, limit }`: what the mechanical engine would do, without doing it.
...
Signals: tangle and ways through (UntangleIt's parse); density and coverage per line (DeepTest's record or silent command, read through the records channel per toolkit-api.md); LCOM4 (UntangleIt's parse of the class); and change history from git, which is already on disk and costs nothing: how often a method changes, and whether a switch has grown case by case in separate commits. History is the only way to tell a fixed list from an extension point.
...
The recommendation is one line on the card above the buttons. It lives in UntangleIt because it is about what to untangle and how; DeepTest stays the judge of tests, KeepSafe stays the undo. Nothing in DeepTest changes except that "Test it first" can arrive from a sibling.
...
3. Where does behaviour verification stop: the existing suite only, or should UntangleIt generate characterization tests before transforming a method that has none?
...
5. Naming: settled on 2026-09-11 as "UntangleIt" after the Marketplace refused the first choice (see the note at the top). One word, capital U, capital I.
...
Every fact above that is not the toolkit's own design comes from one of these. Links were fetched and checked on 2026-09-06; the Marketplace links on 2026-09-11.
...
- Publisher mynkow's Visual Studio refactoring extension. *Visual Studio Marketplace*. https://marketplace.visualstudio.com/items?itemName=mynkow.RefactorIt. (The existing Visual Studio extension sharing the tool's first name; version 1.4.0, about 2,700 installs, changelog ending at VS2017 support.)
- "Publishing Extensions." *Visual Studio Code API*, Microsoft. https://code.visualstudio.com/api/working-with-extensions/publishing-extension. (README image links rewritten to the repository URL, the https and no-SVG rules, the `icon` field, and `.vscodeignore`.)
