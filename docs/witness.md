...
| `W.c(id, i)` | the i-th way through a decision was taken: an arm of an `if` or a ternary, an operand of a boolean run, a default parameter value, a case of a switch |

`W.v` is the only one that returns anything, and that is the only place a counter sits around a value. Everywhere else the counter sits beside the value, in the arm that ran or as the first operand of a comma expression, and never around the condition that chose it. That is not a matter of taste. A condition wrapped in a call stops TypeScript narrowing, so `if (W.b(0, p))` leaves `p` possibly undefined in the body and strict code refuses to compile, and `typeof v === 'string'` wrapped the same way loses the union refinement, which no compiler option can restore. Until 1.0.17 the counters did wrap conditions, and nothing caught it because Angular is the only runner that type-checks what Witness emits. In a TypeScript file the prologue also declares the handle with a real type rather than casting to `any`, or `W.v` would hand back `any` and erase the type of every value it wrapped.
...
Four shapes need care, and each has a test. A bare statement body (`if (x) return;`, `for (...) f();`) is wrapped in braces so the counter sits inside it. An `else if` is braced, `else {W.c(0, 1); if (...)}`, because the arm counter has to go somewhere and `else W.c(0, 1); if (...)` is a different program; its own statement counter still rides in front of its condition as the first operand of a comma, `if ((W.s(2), b))`, which is a place a comma is safe and a call is not. An `if` with no `else` is given one, `else {W.c(0, 1);}`, so the way not taken is still recorded, and that `else` is emitted as part of the consequent's own closing edit, because an enclosing block can end at the same offset and two independent edits there cannot be ordered correctly. A loop under a label carries no counter of its own, because the label must sit directly on its loop for `continue label` to parse; the labeled statement's counter is on the same line. A directive (`'use strict'`) is not a statement to Istanbul or to the engine, and a counter in front of it would demote it to a plain string, so it is left alone and the file's prologue (`const W = globalThis.__witness__.file("<id>")`) is inserted after the directives, or after a shebang.
